...
void inserir(No** cabeca, Livro livro) {
    ...
    if (*cabeca == NULL || strcmp((*cabeca)->livro.titulo, livro.titulo) > 0) {
        ...
        *cabeca = novo;
        ...
        No* atual = *cabeca;
        while (atual->proximo != NULL && strcmp(atual->proximo->livro.titulo, livro.titulo) < 0) {
            atual = atual->proximo;
        }
...
void buscarPorTitulo(No* cabeca, char* titulo) {
    No* atual = cabeca;
...
void buscarPorAutor(No* cabeca, char* autor) {
    No* atual = cabeca;
    int encontrado = 0;
    ...
    do {
        ...
        switch (opcao) {
            ...
            case 4:
                ...
                scanf(" %[^\n]", titulo);
                buscarPorTitulo(cabeca, titulo);
                break;
            case 5:
                printf("Entre com o nome do autor: ");
                scanf(" %[^\n]", autor);
                buscarPorAutor(cabeca, autor);
                break;
            case 6:
                exibirLivros(cabeca);
                break;
